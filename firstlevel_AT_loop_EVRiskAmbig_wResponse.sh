...
do
	...
	HIRES=`awk -v b="$a" 'NR == b+1 {print $11}' ${LOOKUPTABLE}`
	...
	#The lines below submit jobs into the pre-existing pipeline
		...
		qsub -v EXPERIMENT=DECIDE.01 firstlevel_AT_bash_EVRiskAmbig_wResponse.sh $SUBJ $BIAC $DATE $AT1 $HIRES 1
		qsub -v EXPERIMENT=DECIDE.01 firstlevel_AT_bash_EVRiskAmbig_wResponse.sh $SUBJ $BIAC $DATE $AT2 $HIRES 2
		qsub -v EXPERIMENT=DECIDE.01 firstlevel_AT_bash_EVRiskAmbig_wResponse.sh $SUBJ $BIAC $DATE $AT3 $HIRES 3
	fi
